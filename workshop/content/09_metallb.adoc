...
If a node becomes unavailable, the router then initiates a new connection with another node that has a speaker pod which is advertising the load balancer IP address.
...
For this lab we will use the same network where the OpenShift Cluster nodes are located (192.168.123.0/24) and for this exercise we will reserve the IP range 192.168.123.200-192.168.123.250.
...
. Switch to the *L2Advertisement* tab and pressI know you are on the road, have you ever cloned a git repo, and the content you are looking at on the web page, doesn't match the file in front of you, even with everything synced? *Create L2Advertisement*
...
+
[NOTE]
Notice the `type` indicated is `LoadBalancer`, this indicates that the service should use *MetalLB*
...
== Summary 
MetalLB is a great solution for baremetal on-premises deployments to be able to expose applications outside of the cluster, without the need to configure physical networks with multus.

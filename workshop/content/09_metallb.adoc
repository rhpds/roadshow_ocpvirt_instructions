...
. To verify connectivity to the database service via the external IP, open the web terminal clicking in the following icon in the right-top part.
+
...
MetalLB is a straightforward and simple solution for baremetal, on-premises deployments to expose applications outside of the cluster, without the need to configure physical networks with NMstate or multus.

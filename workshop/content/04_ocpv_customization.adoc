...
. Click the link under *CPU | Memory* to adjust the resource allocation for the virutal machine. Change the number of CPUs to 2 and the memory to 4 GiB.
...
* An *affinity rule* is used to indicate that a VM should be scheduled with, or not with in teh case of an antiaffinity rule, other workloads.
...
* The *descheduler* is a feature of OpenShift which will periodically evaluate the VM and the host it's running on to determine if it should be migrated to another host. This could be for resource optimization reasons or because an affinity rule is being violated.
...
+
* The *model* indicates the type of network adapter will be used. VirtIO is the paravirtual NIC, whereas e1000 and others are emulated devices.
...
. Navigate to *Disks* tab to see the devices allocated to the VM.
+
...
+
* The *PersistentVolumeClaim Size* will be the size of the disk attached to the VM. If the source of the disk is another PVC, then it cannot be smaller than the source, otherwise you want to make sure it is at least large enough to hold the QCOW2 or ISO that is being imported.
...
. Navigate to *Scripts* tab. This tab is used to apply guest OS customization, such as cloud-init or Sysprep, at deployment time.
+
...
* *Sysprep* is the Microsoft Windows tool for automatically configuring new OS deployments, including setting configuration like hostname, default `Administrator` passwords, and joining an Active Directory domain.

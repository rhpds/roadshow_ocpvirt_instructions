...
. On the next step select the three VMs: database, winweb01 and winweb02. Haproxy VM can stay unselected as previously explained. Press *Next*:
+
...
. At `webapp` Service details page locate *Pod selector* link and click it
+
image::images/MTV/62_00_VMWARE_VMs_podSelector.png[]

. Verify the two Windows VMs are properly pointed by the Service.
+
